...
mean_and_std <- data[,grep(pattern = "std|mean", x = features, ignore.case = TRUE)]
...
lab <- colnames(data)[-c(562,563)]
data_new <- lapply(X = lab, FUN = function(x) tapply(data[[x]], list(data$activity, data$subject), mean))
names(data_new) <- lab
capture.output(data_new, file = "tidy_data.txt")
